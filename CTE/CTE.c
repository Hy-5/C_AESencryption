...
int fileCompression(const char *source, const char *dest)
{
    ...
    //char tempInputFileString [128] ="s1encryptedOutput";
    strncpy(extension, source+strlen(source)-4,4);
    extension[4]='\0';
    printf("\nExtension is %s\n", extension);
...
int decompress(const char* s2compressedOutput, const char* s3DecompressedOutput) {
    ...
    //printf("\nExtension is now %s\n", extension);
    strcpy(finalFileName, s3DecompressedOutput);
    strcat(finalFileName,extension);
    //printf("TEST %s \n", finalFileName);
    FILE* outFile = fopen(finalFileName, "wb");
...
void encryption(char* inputFileStr){
    ...
    char tempInputFileString [128] ="s1compressedOutput";
    //printf("\nGOTBACKHERE : %s\n", inputFileStr);
    ...
    FILE *encryptedFile = fopen("s2encryptedOutput", "wb");
    ...
    while (1) {
        ...
        switch(choice){
            case 1:
                ...
                fileCompression("s1encryptedOutput", "s1compressedOutput");
                break;
            case 2:
                printf("Running encryption\n");
                encryption("s1compressedOutput");
                break;
            case 3:
                printf("Running decryption\n");
                decryption("s2encryptedOutput");
                
                break;
            case 4:
                printf("Running Decompression\n");
                decompress("s3decryptedOutput", "s4DecompressedOutput");
                break;
            case 5:
                return 0;
                break;
